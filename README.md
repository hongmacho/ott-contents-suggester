...
│  │   (v + m)         (v + m)                           │   │
│  │                                                     │   │
│  │  v = vote_count   m = 200 (신뢰 기준)              │   │
│  │  R = vote_average C = 7.0 (전체 평균)              │   │
│  └─────────────────────────────────────────────────────┘   │
│                           +                                 │
│  선호국가 가산점                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  선호국가 설정 없음          → +0.0                 │   │
│  │  original_language 일치      → +1.5                 │   │
│  │  original_language 불일치    → +0.0                 │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  최종 점수 = Bayesian Rating + 선호국가 가산점              │
└────────┬────────────────────────────────────────────────────┘
         │ 최종 점수 내림차순 정렬
         ▼
┌─────────────────────────────────────────────────────────────┐
│                    최신작 다양성 보장                        │
│                                                             │
│  상위 12개 결과 중 최근 2년 이내 작품이 3개 미만이면        │
│  최하위 구작을 최신작(점수 상위)으로 교체 후 재정렬         │
│                                                             │
│  → 오래된 검증작만 반복 추천되는 문제 방지                  │
└────────┬────────────────────────────────────────────────────┘
         │ 상위 12개 확정
         ▼
┌─────────────────────────────────────────────────────────────┐
│                   추천 이유 생성 (AI)                        │
│                                                             │
│  ANTHROPIC_API_KEY 있음  → Claude Haiku 생성               │
│  ANTHROPIC_API_KEY 없음  → 평점 기반 한국어 폴백           │
